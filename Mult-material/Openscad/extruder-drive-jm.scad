...
bite=-0.5;
motor_hole_pitch=31;
tongue=true; // Set false to get M5 nut bowden retainer
...
rotate([0,180,0])	large_gear();
//echo("gear sep",sqrt(pow(filament_offset_x+filament_d/2+5.6/2-bite,2)+pow(drive_offset_y,2)));

module drive_block(){
	difference(){
		union(){
			translate([0,5-1.25,7]) cube([motor_hole_pitch+8,motor_hole_pitch+15.5,14],center=true);
			translate([17,24,3]) rotate([0,-90,0]){
				cylinder(r=3,h=40);
				translate([-3,0,0]) cube([3,3,40]);
			}
				//boss for carriage mount hole
				rotate([90,0,0]) translate([19,7,(motor_hole_pitch-7)/2]) rotate([0,0,22.5]) cylinder(r=14*da8,h=7.5,$fn=8);
		...
		translate([filament_offset_x,0,7.5]) rotate([90,0,0]){
			...
			{
// Tongue retainer 
				translate([0,0,11.1-0.5]) 
				{
					cylinder(r=5.5/2,h=9);
					translate([0,6,2])
					cube([9,20,4],center=true);
				}
			} else
			{
// M5 nut retainer
				translate([0,0,11.1+5]) 
				{
